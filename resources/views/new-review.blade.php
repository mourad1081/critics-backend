...
@section('content')
    <div class="container-fluid" style="margin-bottom: 100px;">
        <form class="row"
              method="post"
              action="{{ route('save-review', ['id_form' => $id_type_review]) }}"
              id="review"
              data-form-definition-id="{{ $id_type_review }}"
            ...
            <div class="col-12 p-2 p-sm-4 shadow" style="background: white; border-top: solid #EEE 15px;">
                ...
                @foreach($review['sections'] as $section)
                    <div>
                        <h6 style="position:relative;" class="section text-uppercase" data-section-definition-id="{{ $section['section_definition_id'] }}">
                            <button class="btn btn-primary" style="border-radius: 50%;"><i class="fa fa-times"></i></button>
                            <strong class="ml-2">{{ $section['title'] }}</strong>
                        </h6>
                        <hr>
                        @foreach($section['criteria'] as $criterion)
                            <div class="criterion p-3 mb-3"
                                 style="background-color: #eee;"
                                 data-section-definition-id="{{ $section['section_definition_id'] }}"
                                 data-criterion-definition-id="{{ $criterion['criterion_definition_id'] }}">
